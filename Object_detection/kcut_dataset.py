    ...
    def kcut(self):
        ...
        while len(objects) != 0:
            ...
            fore_objects, back_objects, fore, back, fore_plt, back_plt = self.norm_cut(objects[0], visualize=False,
                                                                   use_eigenvec=True,
                                                                   use_ncut_value=False)
            assert len(fore_objects) == len(fore_plt)
            assert len(back_objects) == len(back_plt)
            if fore_objects:
                for fo in fore_objects:
                    fore_box = self.get_objects_coordinate(fo)
                    boxes.append(list(fore_box))
                for fp in fore_plt:
                    maskes.append(fp)
            if back_objects:
                for bo in back_objects:
                    back_box = self.get_objects_coordinate(bo)
                    boxes.append(list(back_box))
                for bp in back_plt:
                    maskes.append(bp)
            ...
            if fore_objects:
                for fo in fore_objects:
                    ...
                    idx_list = np.ravel_multi_index(fo, self.dims)
                    count = np.sum(self.pseudo_labels[idx_list])
                    purity = count / len(idx_list)
                    if purity < 0.2:
                        continue
            ...
            if back_objects:
                for bo in back_objects:
                    ...
                    idx_list = np.ravel_multi_index(bo, self.dims)
                    count = np.sum(self.pseudo_labels[idx_list])
                    purity = count / len(idx_list)
                    if purity < 0.2:
                        continue
...
if __name__ == "__main__":
    ...
    # img_path = '/home/ybmiao/data/INSTRE/INSTRE-S1/47b_wierd_fish/022.jpg'
    # img_path = '/home/ybmiao/data/INSTRE/INSTRE-S1/01a_canada_book/001.jpg'
    ...
    # graphv.extract_dataset(dataset="instre", save_path="/home/ybmiao/output/kcut/10_20")
    # graphv.extract_dataset(dataset="Ins160", save_path="/home/ybmiao/output/kcut/10_20")
    # graphv.extract_dataset(dataset="Ins335", save_path="/home/ybmiao/output/kcut/10_20")

    boxes, maskes = graphv.kcut()
    for mask in maskes:
        plt.imshow(mask)
        plt.show()
