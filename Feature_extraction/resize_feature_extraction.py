...
class Resize_Feature_Extraction():
    ...
    def method_crop(self, layer, boxes, rate, pooling, pic_name, raw_img, is_qry=None):
        assert is_qry is not None
        features = []
        box_lines = []
        for box_idx, box in enumerate(boxes):
            ...
            resize_img = raw_img.crop((xmin, ymin, xmax, ymax))
            ...
            # print(resize_img.size[0]*resize_img.size[1])
            factor = 1
            while (resize_img.size[0]/factor)*(resize_img.size[1]/factor) > 50000:
                factor = factor + 1
            # print(factor)
            img_trans = T.Compose([
                T.Resize((int(resize_img.size[1] / factor), int(resize_img.size[0] / factor))),
                T.ToTensor(),
                T.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
            ])
            if min(resize_img.size[0]/factor, resize_img.size[1]/factor) < self.patch_size:
                img_trans = T.Compose([
                    T.Resize(self.patch_size),
                    T.ToTensor(),
                    T.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225)),
                ])
            input_var = img_trans(resize_img).unsqueeze(0).cuda()
            # print(input_var[0].shape)
            # if factor > 1:
            #     plt.imshow(input_var[0].permute(1, 2, 0).cpu().numpy().astype(float))
            #     plt.show()
            ...
            if self.net == "resnet50":
                ...
                # output, attens = self.model.get_last_output_and_selfattention(input_var)
                output = self.model(input_var)
            else:
            ...
            # if pooling == 'mean':
            #     feature = torch.mean(output, dim=[2, 3]).reshape(-1).cpu().numpy()
            # elif pooling == 'max':
            #     feature, _ = torch.max(output, dim=-2, keepdim=True)
            #     feature, _ = torch.max(feature, dim=-1)
            #     feature = feature.reshape(-1).cpu().numpy()
            # elif pooling == "gem":
            #     fea_pow = torch.pow(output, 3)
                ...
                for layer in self.layers:
                    for pooling in self.pooling:
                        ...
                        if method == 'roi':
                            ...
                            features, boxes_list = self.method_crop(layer,boxes,rate,pooling, pic_name,raw_img, is_qry)
                        else:
...
# parse.add_argument('--save_path', type=str, default='/media/media01/ybmiao/output/eig_back/6_29/Ins160/testtest')
...
# parse.add_argument('--save_path', type=str, default='/media/media01/ybmiao/output/eig_back/6_29/instre/testtest')

# parse.add_argument('--dataset', type=str, default='Ins160')
# parse.add_argument('--box_path', type=str, default='/home/ybmiao/output/kcut/12_07/Ins160/box_ref.txt')
# parse.add_argument('--save_path', type=str, default='/home/ybmiao/output/kcut/12_07/Ins160/original_crop_test')
parse.add_argument('--dataset', type=str, default='instre')
parse.add_argument('--box_path', type=str, default='/home/ybmiao/output/kcut/12_07/instre/box_ref.txt')
parse.add_argument('--save_path', type=str, default='/home/ybmiao/output/kcut/12_07/instre/test2/original_crop_test')

# parse.add_argument('--dataset', type=str, default='instre')
# parse.add_argument('--box_path', type=str, default='/home/ybmiao/output/kcut/10_20/instre/box_ref.txt')
# parse.add_argument('--save_path', type=str, default='/home/ybmiao/output/kcut/10_20/instre/test2/masked_dino')
# parse.add_argument('--dataset', type=str, default='instre')
# parse.add_argument('--box_path', type=str, default='/media/media01/ybmiao/output/EMB/7_22/instre/box_ref.txt')
# parse.add_argument('--save_path', type=str, default='/media/media01/ybmiao/output/EMB/7_22/instre/testtest')
# parse.add_argument('--dataset', type=str, default='Ins160')
# parse.add_argument('--box_path', type=str, default='/media/media01/ybmiao/output/EMB/7_22/test/Ins160/box_ref.txt')
# parse.add_argument('--save_path', type=str, default='/media/media01/ybmiao/output/EMB/7_22/Ins160/testtest')
parse.add_argument('--para', type=str, default='pretrained')  # swav
parse.add_argument('--mode', type=str, default='all')  # qry / ref / all
# parse.add_argument('--mode', type=str, default='qry')  # qry / ref / all
...
parse.add_argument('--pooling', type=str, nargs='+', default=['mean'])  # 'max')
...
parse.add_argument('--methods', type=str, nargs='+', default=['crop'])
...
if __name__ == "__main__":
    ...
    resize_feature_extract = Resize_Feature_Extraction(use_config=args.use_config, show_info=args.show_info, gpu=args.gpu, dataset=args.dataset, box_path=args.box_path,
    ...
    resize_feature_extract.extract()
