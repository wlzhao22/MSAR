...
class Feature_Process():
    ...
    def retrieval(self, folder_path, is_whiten, topk, threshold, dataset):
        ...
        qName, qCod, qMat, rName, rCod, rMat, grdDict = self.load_retrieval_data(folder_path, is_whiten=False, dataset=self.dataset)
...
if __name__ == "__main__":
    ...
    if args.function == "gen_pca_pkl":
        ...
        if args.dataset == 'CUHK-SYSU':
            feature_process.retrieval_and_map(topk=[10], is_save=False, is_whiten=False)
        else:
            feature_process.retrieval_and_map(topk=[50, 100, 0], is_whiten=False)
    else:
